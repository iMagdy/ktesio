...
Fleet detail surfaces the ledger honestly: `usage` shows real cumulative and current-Run **token** totals that equal the ledger exactly, and the active Metering Source is visible. It renders **tokens only** — there is no dollar figure until a Rate exists (Epic 3.3), and no budget/headroom until budgets land (Epic 3.2); those cells stay a typed absence. Because this touches the adapter-facing metering surface (the documented usage-line channel), the Adapter Contract version took an additive minor bump.
